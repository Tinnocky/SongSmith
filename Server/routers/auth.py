...
from Server.routers.utils import get_user_manager, validate_strong_password, create_access_token, \
    create_refresh_token, get_user_data, get_refresh_token_data
...
async def register(request: UserRequest, users_table=Depends(get_user_manager)):
    """check if a username is registered, if it is then raise an exception. if not, register it."""
    pw_error_messages = await validate_strong_password(request.password)
...
@router.post("/login")
def login(request: UserRequest, users_table=Depends(get_user_manager)) -> LoginResponse:
    ...
    access_token = create_access_token(user.id, user.username)  # grant access token
    refresh_token = create_refresh_token(user.id, user.username)  # grant refresh token
...
def delete_user(user_data: dict = Depends(get_user_data), users_table=Depends(get_user_manager)):
...
def refresh(user_data: dict = Depends(get_refresh_token_data)) -> RefreshResponse:
    """remove the old access token and issue a new access token to the client so they don't get logged out"""
    new_token = create_access_token(user_data["user_id"], user_data["username"])
...
async def change_password(request: ChangePasswordRequest, user_data: dict = Depends(get_user_data),
                          users_table=Depends(get_user_manager)):
    ...
    pw_error_messages = await validate_strong_password(request.new_password)
    if pw_error_messages:  # if not None = password is bad
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST,
                            detail="\n".join(pw_error_messages))
